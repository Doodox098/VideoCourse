    ...
    for idx, frame in tqdm(enumerate(frames), leave=False):
        ...
        if prev_frame is not None:
            ...
            if idx > frames_seq_len:
                ...
                if metric_values[idx - ((frames_seq_len + 1) // 2) + 1].astype(float) / mean.astype(
                        float) > metrics_threshold or metric_values[
                    idx - ((frames_seq_len + 1) // 2) + 1] > hard_metrics_threshold:
